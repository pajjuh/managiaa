...
def dashboard():
    ...
    if role == 'admin':
        ...
        my_project_ids = [m['project_id'] for m in member_res.data]
        projects = supabase.table('projects').select('*').in_('id', my_project_ids).execute().data or []
        # Non-admins see tasks assigned to them OR tasks in their projects (optional, but let's stick to assigned for now as per original logic)
        tasks = [t for t in all_tasks if t.get('assigned_to') == user_id]
...
def task_list():
    ...
    # Get supporting data
    if role == 'admin':
        projects = supabase.table('projects').select('id,name').execute().data or []
    else:
        member_res = supabase.table('project_members').select('project_id').eq('user_id', user_id).execute()
        my_project_ids = [m['project_id'] for m in member_res.data]
        projects = supabase.table('projects').select('id,name').in_('id', my_project_ids).execute().data or []
    
    users = supabase.table('users').select('id,full_name').execute().data or []
    user_map = {u['id']: u['full_name'] for u in users}
    project_map = {p['id']: p['name'] for p in projects}

    return render_template('tasks/list.html', tasks=tasks, projects=projects,
                           users=users, user_map=user_map, project_map=project_map,
...
def task_create():
    ...
    if role == 'admin':
        projects = supabase.table('projects').select('id,name').eq('status', 'active').execute().data or []
    else:
        member_res = supabase.table('project_members').select('project_id').eq('user_id', user_id).execute()
        my_project_ids = [m['project_id'] for m in member_res.data]
        projects = supabase.table('projects').select('id,name').eq('status', 'active').in_('id', my_project_ids).execute().data or []
    ...
    if request.method == 'POST':
        ...
        # Security check: Ensure user is member of the project
        if project_id and role != 'admin':
            member_check = supabase.table('project_members').select('id').eq('project_id', int(project_id)).eq('user_id', user_id).execute()
            if not member_check.data:
                flash('Unauthorized project selection.', 'danger')
...
def task_detail(task_id):
    ...
    # Security Check: Must be admin, assignee, or project member
    if role != 'admin' and task.get('assigned_to') != user_id:
        if task.get('project_id'):
            member_check = supabase.table('project_members').select('id').eq('project_id', task['project_id']).eq('user_id', user_id).execute()
            if not member_check.data:
    ...
    users = supabase.table('users').select('id,full_name,role').execute().data or []
    ...
    return render_template('tasks/detail.html', task=task, comments=comments,
                           ...
                           project_map=project_map, users=users, projects=projects)
...
def task_edit(task_id):
    ...
    # Only admin or creator/assignee can edit (let's say admin or creator for now)
    if role != 'admin' and task.get('created_by') != user_id:
    ...
    if request.method == 'POST':
        ...
        if not data['title']:
            flash('Task title is required.', 'danger')
            return render_template('tasks/create.html', task=task, projects=projects, users=users, editing=True)
...
def task_status_update(task_id):
    ...
    res = supabase.table('tasks').select('id,assigned_to,project_id').eq('id', task_id).execute()
    if not res.data:
        flash('Task not found.', 'danger')
        return redirect(url_for('task_list'))
    task = res.data[0]
    
    # Check permission: admin, assignee, or project member
    is_allowed = role == 'admin' or task.get('assigned_to') == user_id
    if not is_allowed and task.get('project_id'):
        member_check = supabase.table('project_members').select('id').eq('project_id', task['project_id']).eq('user_id', user_id).execute()
        is_allowed = bool(member_check.data)
...
def project_list():
    ...
    if role == 'admin':
        projects = supabase.table('projects').select('*').order('created_at', desc=True).execute().data or []
    else:
        member_res = supabase.table('project_members').select('project_id').eq('user_id', user_id).execute()
        my_project_ids = [m['project_id'] for m in member_res.data]
        projects = supabase.table('projects').select('*').in_('id', my_project_ids).order('created_at', desc=True).execute().data or []
    tasks = supabase.table('tasks').select('id,project_id,status').execute().data or []
    users = supabase.table('users').select('id,full_name').execute().data or []
    user_map = {u['id']: u['full_name'] for u in users}
...
def project_detail(project_id):
    ...
    tasks = supabase.table('tasks').select('*').eq('project_id', project_id).order('created_at', desc=True).execute().data or []
    members = supabase.table('project_members').select('*').eq('project_id', project_id).execute().data or []
    users = supabase.table('users').select('id,full_name,role').execute().data or []
    user_map = {u['id']: u for u in users}
    ...
    return render_template('projects/detail.html', project=project, tasks=tasks,
                           members=members, users=users, user_map=user_map,
...
def member_list():
    ...
    tasks = supabase.table('tasks').select('id,assigned_to,status').execute().data or []
    for u in users:
    ...
    return render_template('members/list.html', users=users)
...
def profile():
    ...
    my_tasks = supabase.table('tasks').select('*').eq('assigned_to', user_id).order('created_at', desc=True).execute().data or []
